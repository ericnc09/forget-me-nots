...
- `channels.mts` holds the declaration and handlers. `channel-host.mts` owns lifecycle. `app/agent.ts` creates isolated A2A agent runs and supports clone(). Preserve that isolation.
...
- Google orchestrator and analysis models default to gemini-3.6-flash. Respect ORCHESTRATOR_MODEL and GEMINI_MODEL overrides. Verify account access before changing defaults.
- The Python code uses A2A SDK 0.3 server APIs. Do not install the unrelated `a2a` distribution or upgrade to A2A SDK 1.x without porting and verifying both servers.
