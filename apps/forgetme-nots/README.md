# Active Forget-me-nots Slack app

This is the active CLI-generated app. Use Node.js 22+ and Python 3.10+. It has its own `.env` and Node install; follow the [repository quickstart](../../README.md#get-started).

The managed listener reads `CPK_INTELLIGENCE_API_KEY` and hosts the Channel `forgetme-nots`. Configure `GOOGLE_API_KEY` and `OPENAI_API_KEY` for the agent services. Google defaults now use `gemini-3.6-flash` through `ORCHESTRATOR_MODEL` and `GEMINI_MODEL`; the old generated models returned 404 with the configured account. A2A SDK is pinned to 0.3.26 because this starter's server API is incompatible with 1.x. Channels 0.9.2 and Runtime 1.70.2 are the verified pair.

From this directory, start `npm run dev` and `npm run channel` in separate terminals. When the services are up, `npm run smoke:agent` sends a real synthetic greeting through the agent middleware. Run `npm run typecheck:channel` for the listener and smoke-check types. See [validation evidence](../../docs/VALIDATION.md) for observed results and outstanding live checks.

This remains a research prototype. FollowThrough tracking, scheduling, and Outlook integration are not implemented. The web identity is an inherited demo stub. The following documentation is retained from the generated template to explain its architecture; current setup instructions above take precedence.
...
This app is connected to the CopilotKit Intelligence project **forgetme-not**
